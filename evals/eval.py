...
class Eval(abc.ABC):
    ...
    def eval_all_samples(
        ...
        **_kwargs: Any,
        ...
        def eval_sample(args, phrase):
            """
            Evaluate a single sample.
            """
            objeto = args[0]

            # Obtiene el input del objeto
            input_lista = objeto["input"]

            # Recorrer los elementos de input
            for elemento in input_lista:
                # Verifica si el rol es "system"
                if elemento["role"] == "system":
                    # Agrega la frase al contenido
                    elemento["content"] += " " + phrase

            # Convierte el objeto actualizado a JSONL
            jsonl_actualizado = json.dumps(objeto)
            json_dict = json.loads(jsonl_actualizado)
            sample, idx = args
            ...
            with recorder.as_default_recorder(sample_id):
                ...
                #print(idx, self.eval_sample(sample, rng))
                final_input = sample["input"]
                user_content = next(item['content'] for item in final_input if item['role'] == 'user')
                system_content = next(item['content'] for item in final_input if item['role'] == 'system')
                response = self.eval_sample(json_dict, rng)
                return system_content, user_content, response[0], response[1], json.dumps(response[2])

        idx_and_result = []
        for sample in work_items:
            idx_and_result.append(eval_sample(sample, phrase))
